...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let isValidNumber = number => ! isNaN(number);
  let isDivisibleByFive = number => number % 5 === 0;

  let result = input.reduce((outputArray, element) => {
    outputArray.push(element.filter(isValidNumber).filter(isDivisibleByFive).reduce((newArray, element) => {
      newArray.push(Math.pow(2, element)); //Math.pow(2, element));
      return newArray;
    }, [] ));
    return outputArray;
  }, [] );
...
let findMaleAndFemale = (data) => {
  ...
  let andStr = ' and ';
  // data.map(element[i] => {
  //   if (element[i].gender === 'male' || element[i].gender === 'female') {
  //     result = element[i].name.concat(and);
  //   }
  // });

  console.log(result);
  // result.map(Object.values(arr)[0].name);
