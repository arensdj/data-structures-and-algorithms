...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let counter = arr.reduce((accumulator) => accumulator += 1);
  return counter;
};
...
const reversedString = (arr) => {
  ...
  let result = newArray.reduce((accumulator, current) => current.concat(accumulator));
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  let counter = arr.reduce((total, element) => {
    if (isPrime(element)) {
      total += 1;
    }
    return total;
