...
const isCapitalized = (str) => {
  // Solution code here...
  // let testRegex = /\b[A-Z].*?\b/;  //[A-Z]\w+
  let array = [];
  let testReg = /([A-Z][a-z]+)/;
  let words = str.split(' ');

  words.forEach(elem => {
    if (testReg.test(elem)) {
      array.push(elem);
    }
  });
...
const matchMonth = (input) => {
  // Solution code here...
  // let testReg = /Oct(ober)?/gi;
  let testReg = /^[Oo][c][t](ober)*$/;
  // console.log(testReg.test(input));
  return testReg.test(input);
};
